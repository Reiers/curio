...
func verifySettle(ctx context.Context, db *harmonydb.DB, ethClient ethchain.EthClient, fwssv *FWSS.FilecoinWarmStorageServiceStateView, fwssAddr common.Address, settle settled) error {
	...
	comm, err := db.BeginTransaction(ctx, func(tx *harmonydb.Tx) (commit bool, err error) {
		for _, railId := range settle.Rails {
			...
			if getRailErr != nil {
				if !filecoinpayment.IsRailInactiveOrSettledError(getRailErr) {
				...
				finalized, finErr := pipelineFinalizationElapsed(tx, dataSet.Int64(), current)
				if finErr != nil {
				...
				if err := ensureDataSetDeletion(tx, dataSet.Int64()); err != nil {
					return false, xerrors.Errorf("failed to ensure deletion of dataSet: %w", err)
				}
				continue
			}
			...
			if view.EndEpoch.Int64() > 0 {
				...
				if err := FWSS.EnsureServiceTermination(tx, dataSet.Int64()); err != nil {
					log.Errorw("failed to ensure service termination", "dataSetId", dataSet.Int64(), "railId", railId, "error", err)
					return false, xerrors.Errorf("failed to ensure service termination: %w", err)
				}
...
func ensureDataSetDeletion(tx *harmonydb.Tx, dataSetID int64) error {
	n, err := tx.Exec(`UPDATE pdp_delete_data_set SET deletion_allowed = TRUE WHERE id = $1`, dataSetID)
	if err != nil {
...
func pipelineFinalizationElapsed(tx *harmonydb.Tx, dataSetID int64, current uint64) (bool, error) {
	type row struct {
