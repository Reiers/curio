...
func (P *PDPV0IPNITask) Do(ctx context.Context, taskID harmonytask.TaskID, stillOwned func() bool) (done bool, err error) {
	...
	for range ipniHeadCASRetries {
		...
		var prev string
		err = P.db.QueryRowI(ctx, `SELECT head FROM ipni_head WHERE provider = $1`, task.Prov).Scan(&prev)
		if err != nil && !errors.Is(err, pgx.ErrNoRows) {
			return false, xerrors.Errorf("querying previous head: %w", err)
		}

		var privKey []byte
		err = P.db.QueryRowI(ctx, `SELECT priv_key FROM ipni_peerid WHERE sp_id = $1`, PDP_v0_SP_ID).Scan(&privKey)
		if err != nil {
