...
func processPendingDeletes(ctx context.Context, network contract.Network, db harmonyquery.DBInterface, ethClient ethchain.EthClient) error {
	...
	successErr := processSuccessfulDeletes(ctx, network, db, ethClient, successes)
...
func processSuccessfulDeletes(ctx context.Context, network contract.Network, db harmonyquery.DBInterface, ethClient ethchain.EthClient, successes []pendingDataSetDelete) error {
	if len(successes) == 0 {
		return nil
	}
...
func processFailedDeletes(ctx context.Context, db harmonyquery.DBInterface, failures []pendingDataSetDelete) error {
...
func cleanupDeletedDataSet(ctx context.Context, db harmonyquery.DBInterface, dataSetID int64, deleteTxHash string) error {
	comm, err := db.BeginTransactionI(ctx, func(tx harmonyquery.TxInterface) (commit bool, err error) {
		...
		*/
		...
		_, err = tx.ExecI(`DELETE FROM pdp_data_sets WHERE id = $1`, dataSetID)
		if err != nil {
			return false, xerrors.Errorf("failed to delete data set %d: %w", dataSetID, err)
		}

		_, err = tx.ExecI(`DELETE FROM  pdp_delete_data_set WHERE id = $1 AND delete_tx_hash = $2`, dataSetID, deleteTxHash)
		if err != nil {
