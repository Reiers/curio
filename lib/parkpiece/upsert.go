...
func UpsertSkipWithInserted(tx harmonyquery.TxInterface, pieceCID string, paddedSize, rawSize int64, longTerm, skip bool) (int64, bool, error) {
	indexValid, err := ActiveIndexValid(tx)
	if err != nil {
...
func upsertFallbackWithInserted(tx harmonyquery.TxInterface, pieceCID string, paddedSize, rawSize int64, longTerm bool, skip *bool) (int64, bool, error) {
	var id int64
	err := tx.QueryRowI(`
		SELECT id FROM parked_pieces
		WHERE piece_cid = $1 AND piece_padded_size = $2 AND long_term = $3 AND cleanup_task_id IS NULL
